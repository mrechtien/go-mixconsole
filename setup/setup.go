...
func Bootstrap() *config.Config {
	...
	for _, mapping := range cfg.Mappings {
		displayEvents <- display.CreateDisplayEvent(mapping.Target, uint8(1))
	}
	...
	var mixgoCfg config.Config
	for event := range inputEvents {
		for _, mapping := range cfg.Mappings {
			if mapping.Control == event.Control {
				slog.Info("loading matching config", slog.Any("config", mapping.Config))
				mixgoCfg = config.ReadConfig("config/" + mapping.Config)
				inputSource.Reset()
	...
	log.Fatal("Failed to bootstrap MixGo config")
	return nil
}
